extern crate rand;

use choice::Choice;
use choice::Event;
use player::Player;
use players::Players;
use trade_row::TradeRow;

mod choice;
mod card;
mod player;
mod players;
mod trade_row;

fn main() {
    let mut trade_row = TradeRow::new();
    let mut player_1 = Player::new("Cameron");
    let mut player_2 = Player::new("Josh");
    let mut players: Vec<&mut Player> = Vec::new();

    players.push(&mut player_1);
    players.push(&mut player_2);

    let mut events: Vec<Event> = Vec::new();
    let mut turn_count = 0;

    while players.iter().all(|ref p| p.authority > 0)
    {
        let (first_player, second_player) = players.split_at_mut(1);
        let first_players_turn = turn_count % 2 == 1;
        let mut current_player: &mut Player;
        let mut opponents: &mut [&mut Player];
        if first_players_turn {
            current_player = first_player[0];
            opponents = second_player;
        } else {
            current_player = second_player[0];
            opponents = first_player;
        }

        print!("Turn {}\n", turn_count);
        print!("{:#}", current_player);

        current_player.begin_turn();
        loop {
            let choice = current_player.make_choice(&trade_row, opponents);
            match choice {
                Choice::EndTurn => break,
                _ => {
                    let event = choice.choose(&mut current_player,
                                              opponents,
                                              &mut trade_row);
                    events.push(event);
                }
            };
        }
        current_player.end_turn();


        turn_count = turn_count + 1;
        print!("\n{:#}", trade_row);
    }

    println!("{:?}", events);
}
